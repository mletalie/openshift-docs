...
[IMPORTANT]
====
Before upgrading a Workload Identity Federation (WIF)-enabled {product-title} on {GCP} cluster, you must update the wif-config. For more information, see xref:../osd_gcp_clusters/creating-a-gcp-cluster-with-workload-identity-federation.adoc#wif-configuration-update_osd-creating-a-cluster-on-gcp-with-workload-identity-federation[Updating a WIF configuration].
====
...
Before updating a {product-title} on {GCP} cluster with WIF authentication type to a newer y-stream version, you must update the wif-config to that version as well. Failure to do so before attempting to update the cluster version will result in an error.
For more information on how to update a wif-config, see xref:../osd_gcp_clusters/creating-a-gcp-cluster-with-workload-identity-federation.adoc#wif-configuration-update_osd-creating-a-cluster-on-gcp-with-workload-identity-federation[Updating a WIF configuration].
