...
Before creating the cluster, you must first create a WIF configuration. You can create a WIF configuration in auto or manual mode.
...
.Procedure
You can create a WIF configuration using auto mode or manual mode.

**Auto mode** enables you to automatically create the service accounts for {product-title} components as well as other IAM resources.

Alternatively, you can use **Manual mode* to review the details in regard to the service accounts for {product-title} components and other IAM resources before using your preferred options.

* Based on your preference, run one of the following commands to create a WIF configuration:
...
<4> Optional: Replace `<required_versions>` with the list of Openshift versions that the WIF configuration will need to support. If no value is provided, the latest available version will be supported by default.
+
--
.Example output
[source,terminal]
...
<2> Replace `<gcp_project_id>` with the ID  of the {GCP} project where the WIF configuration will be implemented.
+
Once the WIF is configured, the following service accounts, group and roles are created.
+
...
* Run one of the following commands to create an {product-title} on (GCP) cluster with WIF configuration:
...
--provider=gcp \
--wif-config <wif_name> \ <2>
--region <gcp_region> \ <3>
--authentication-type <service-account|wif>  \ <4>
--compute-machine-type <instance_type> \ <5>
--version <version> <6>
----
<1> Replace `<cluster_name>` with a name for your cluster.
<2> Replace `<wif_name>` with the name of the newly created or pre-existing WIF configuration.
<3> Replace `<gcp_region>` with the {GCP} region where the new cluster will be deployed.
<4> Select "wif" as your authentication type.
<5> Optional: Replace `<instance_type>` with the instance type you want for your cluster.
<6> Optional: Replace `<version>` with {product-title} version you desire for your cluster.
--
+
[NOTE]
====
