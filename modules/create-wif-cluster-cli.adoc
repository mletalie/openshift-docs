...
<3> Optional: Replace `<osd_versions>` with the desired {product-title} versions the wif-config will need to support. If you do not specify versions, the wif-config will support the latest version only.
--
.Example output
[source,terminal]
----
...
[NOTE]
====
A wif-config that is updated to a newer version will be supported by the updated version as well as any previously supported versions.
====
