...
. Log in to {cluster-manager-url} and click *Create cluster* on the {product-title} tile.
...
+
//Admonition does not apply to WIF according to lead developer: https://redhat-internal.slack.com/archives/D05AW2KHFAA/p1727731298245429
//further research for overall docs in regard to KMS service account.
// ====
...
// [IMPORTANT]
// ====
// After your cluster is created, you can change the number of compute nodes, but you cannot change the compute node instance type in a created machine pool. You can add machine pools after installation that use a customized instance type.
// ====
...
[IMPORTANT]
====
The previous step refers to labels within Kubernetes, not Google Cloud. For more information regarding Kubernetes labels, see link:https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/[Labels and Selectors].
====
. Click *Next*.
...
[IMPORTANT]
====
The VPC owner of the host project must enable a project as a host project in their Google Cloud console. For more information, see link:https://cloud.google.com/vpc/docs/provisioning-shared-vpc#set-up-shared-vpc[Enable a host project].
====
...
[IMPORTANT]
====
The VPC owner of the host project must add the *Computer Network Administrator*, *Compute Security Administrator*, and *DNS Administrator* roles to the following service accounts prior to cluster installation:
