...
{product-title} on {GCP} users can deploy a single machine pool in a specific availability zone that is part of a Multi-AZ cluster using the {cluster-manager} CLI (`ocm`). This option is especially useful in the following situations:

- Your desired instance type is not available in all the availability zones of a specific region.
- Your cluster does not need more than one of the desired instance types.
...
<5> Replace `<machine_pool_id>` with an ID for your machine pool.
