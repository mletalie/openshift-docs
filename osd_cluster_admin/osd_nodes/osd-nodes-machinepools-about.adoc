...
[IMPORTANT]
====
You can not assign the default machine pool to a single availability zone during the provisioning of a Multi-AZ cluster.
====
...
<5> Optional: Replace `<machine_pool_id>` with an ID for your machine pool.
